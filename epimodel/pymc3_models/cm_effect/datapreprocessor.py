        ...
        for r in range(len(self.Rs)):
            for f_i, f in enumerate(self.CMs):
                if np.sum(self.ActiveCMs[r, f_i, :]) > 0:
                    ...
                    nz = np.nonzero(self.ActiveCMs[r, f_i, :])
                    # if the first day that the feature is on corresponds to m
                    if np.isnan(self.Confirmed.data[r, nz[0]]) or np.isnan(self.Deaths.data[r, nz[0]]):

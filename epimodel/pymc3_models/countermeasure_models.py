...
class CountermeasureModelV2(BaseCountermeasureModel):
    def __init__(
        ...
        CountryGrowthRate = pm.Lognormal(
            "CountryGrowthRate", pm.math.log(BaseGrowthRate), 0.3, shape=(nCs,)
        )
        ...
        CountryScaleMult = pm.Lognormal(
            "CountryScaleMult", 0.0, 1.0, shape=(nCs,)
        )  # Weak prior!
        ...
        GrowthReduction = pm.Deterministic(
            "GrowthReduction", T.prod(ActiveCMReduction, axis=1)
        )
        ...
        PredictedGrowth = pm.Deterministic(
            "PredictedGrowth",
        ...
        DailyGrowth = pm.Lognormal(
            "DailyGrowth",
            pm.math.log(PredictedGrowth),
            CountryScaleMult.reshape((nCs, 1)) * 0.1,
        ...
        Size = pm.Deterministic(
            "Size", T.reshape(InitialSize, (nCs, 1)) * DailyGrowth.cumprod(axis=1)
        )
        ...
        Observed = pm.Lognormal(
            "Observed",
            pm.math.log(Size),
            CountryScaleMult.reshape((nCs, 1)) * 0.4,
            shape=(nCs, nDs - CMDelayCut),
    ...
    def plot_inferred_cm_effect(self):
        if self.trace is None:
            raise AssertionError(
                "Chain has not been sampled from - please call model.sample()"
            )

...
def test_generate_gleam_batch(datadir):
    ...
    with runner.isolated_filesystem():
        ...
        pd.testing.assert_frame_equal(
            hdf_gen.get("initial_compartments"),
            hdf_val.get("initial_compartments"),
            check_exact=False,
            check_less_precise=True,
        )
        hdf_gen_sim = (
            hdf_gen.get("simulations").reset_index().drop(columns=["SimulationID"])
        )
        hdf_val_sim = (
            hdf_val.get("simulations").reset_index().drop(columns=["SimulationID"])
        )

        # check all other columns
        pd.testing.assert_frame_equal(
            hdf_gen_sim.drop(columns=["DefinitionXML"]),
            hdf_val_sim.drop(columns=["DefinitionXML"]),
            check_exact=False,
            check_less_precise=True,
        )
        ...
        for i in range(len(gen_xml)):
            ...
            root_gen.find(
                "{http://www.gleamviz.org/xmlns/gleamviz_v4_0}definition"
            ).set("id", "null")
            root_gen.find(
                "{http://www.gleamviz.org/xmlns/gleamviz_v4_0}definition"
            ).set("name", "null")
...
def test_export_gleam_batch(datadir):
    ...
    with runner.isolated_filesystem():
        ...
        sims_gen_dirs = sorted(
            filter(lambda f: not f.startswith("."), os.listdir(sims_gen))
        )
        sims_val_dirs = sorted(
            filter(lambda f: not f.startswith("."), os.listdir(sims_val))
        )
        assert len(sims_gen_dirs) == len(sims_val_dirs)

        for i in range(len(sims_gen_dirs)):
            root_gen = ET.parse(
                str(sims_gen / sims_gen_dirs[i] / "definition.xml")
            ).getroot()
            root_gen.find(
                "{http://www.gleamviz.org/xmlns/gleamviz_v4_0}definition"
            ).set("id", "null")
            ...
            root_val = ET.parse(
                str(sims_val / sims_val_dirs[i] / "definition.xml")
            ).getroot()
            root_val.find(
                "{http://www.gleamviz.org/xmlns/gleamviz_v4_0}definition"
            ).set("id", "null")
...
def test_web_export(datadir):
    ...
    with runner.isolated_filesystem():
        ...
        result = runner.invoke(
            ...
            [
                "web-export",
                str(datadir / "files/example-output-batch.hdf5"),
                "data/sources/estimates-JK-2020-04-15-no-CD.csv",
            ],
        )
        assert result.exit_code == 0
        ...
        with open(json_gen / config["gs_datafile_name"], "r") as blob:
            data_gen = json.load(
                blob,
                parse_constant=(
                    lambda x: raise_("Not valid JSON: detected `" + x + "'")
                ),
            )
